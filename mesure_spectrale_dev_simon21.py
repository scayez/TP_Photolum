...
qtCreatorFile = "ui_photolum.ui" # lie le fichier Interface Utilisateur avec ce fichier python
...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):   
    ...
    def __init__(self):     
        ...
        # Gestion du slider qui permet à l'utilisateur de sélectionner le pas entre chaque mesure
    ...
    def initialisation(self):
       ...
       self.com_port_thread = ComPortThread(
           ...
           bitrate=int(self.comboBox_bitrate.currentText()),
      ...
      # try:
       ...
       if erreur_init != 0:
          ...
          # self.pushButton_cancel_init.setEnabled(True)
       else:
    ...
    def calculate_duration(self):
        ...
        self.delay = self.spinBox_delay.value()
        self.total_points = int((self.l_final - self.l_init) / self.pas) + 1
        #reponse = 0.4#temps d'acquisition sans délai
        
        reponse = self.doubleSpinBox_reponse.value() #temps d'acquisition sans délai
        self.duration = (self.total_points*((self.delay/1000)+reponse)) 
        self.duration = round(self.duration,1)
        
        
        # Convert the float duration (in seconds) to hours, minutes, and seconds
        hours = int(self.duration // 3600)
        minutes = int((self.duration % 3600) // 60)
        seconds = int(self.duration % 60)
        # Format the duration as hh:mm:ss
        #formatted_time = f"{hours:02}:{minutes:02}:{seconds:02}"
        formatted_time = f"{hours:02}h {minutes:02}min {seconds:02}s"
    ...
    def stop(self):
        ...
        #self.arret_mesure= True # passe à 1 lors de l’appuie sur stop par l’utilisateur
...
if __name__ == "__main__": # permet de quitter complètement le programme et de fermer les processus
